...
from sftk.email_handler import Email, EmailHandler
...
class TestEmailHandler(TestCase):
    ...
    def setUp(self, mock_imap):
        ...
        self.mock_mail.list.return_value = (
            "OK",
    ...
    def test_login_failure(self):
        ...
        with self.assertRaises(imaplib.IMAP4.error):
            self.handler.login(
                email="wrong@example.com", password="wrongpassword", mailbox="INBOX"
            )
    ...
    def test_search_no_results(self):
        ...
        self.assertEqual(
            result,
            [],
            f"Empty list should be returned when no emails match the search criteria. Got: {result}",
        )
    ...
    def test_get_email_content_fetch_failure(self):
        ...
        self.assertIsNone(
            result, "None should be returned when the email cannot be fetched."
        )
    ...
    def test_get_email_content(self):
        ...
        self.assertEqual(
            email_obj.subject,
            "Test Subject",
            f"The email subject should be 'Test Subject', Got: {email_obj.subject}",
        )
        self.assertEqual(
            email_obj.body,
            "This is a test email",
            f"The email body should be 'This is a test email', Got: {email_obj.body}",
        )
        ...
        self.assertIsNone(
            json_data,
            "No JSON data should be found in the email body, Got: {json_data}",
        )
        ...
        self.assertEqual(
            json_data,
            expected_result,
            f"The extracted JSON data should be {expected_result}, Got: {json_data}",
        )
