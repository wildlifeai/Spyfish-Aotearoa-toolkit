...
class ScientificNameProcessing:
    ...
    def query_api(self) -> ScientificNameEntry:
        ...
        params = { 
            "like": "false",
            "marine_only": "true",
            "offset": 1 # starts at 1
        }
        try:
            response = requests.get(api_url, params=params)
        except requests.exceptions.RequestException as e:
            return self.failed_to_process(f"{self.scientific_name_to_check}, {self.common_name}  was not processed, error connecting to API: {e}", "api error")
        ...
        if not accepted_scientific_name:
            return self.failed_to_process(f"{self.scientific_name_to_check}, {self.common_name} was not processed, no valid_name found in API response.", "no accepted name")
    ...
    def failed_to_process(self, message, status="failure"):
        ...
        return ScientificNameEntry(common_name=self.common_name, scientific_name_to_check=self.scientific_name_to_check, status=status)
